...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'train':
        ...
        if shaped_loss:
            torch.save(ml3_loss.model.state_dict(), 'data/shaped_ml3_loss_mountainc_car.pt')
        else:
            torch.save(ml3_loss.model.state_dict(), 'data/ml3_loss_mountainc_car.pt')

    if sys.argv[1] == 'test':
        shaped_loss = sys.argv[2] == 'True'
        if shaped_loss:
            ml3_loss.model.load_state_dict(torch.load('data/shaped_ml3_loss_mountainc_car.pt'))
        else:
            ml3_loss.model.load_state_dict(torch.load('data/ml3_loss_mountainc_car.pt'))
        ml3_loss.model.eval()
        opt_iter = 2
        args = (torch.Tensor(s_0), torch.Tensor(goal), time_horizon)
        states = test_ml3_loss(policy,ml3_loss, opt_iter,*args)
        if shaped_loss:
            np.save('data/shaped_ml3_mc_states.npy',states)
        else:
            np.save('data/ml3_mc_states.npy', states)

        if shaped_loss:
            env.render(list(np.array(states)[:, 0]), file_path='./plots/shaped_ml3_mc.gif')
        else:
            env.render(list(np.array(states)[:, 0]), file_path='./plots/ml3_mc.gif')
